...
class WikiGameCog(commands.Cog):
    ...
    async def showhelp(self, ctx: discord.Interaction):
        helptext = {
            ...
            The wikipedia's *random article* button may be used but the articles may be less interesting.
...
class GuesserDropdown(discord.ui.Select):
    def __init__(self, game):
        ...
        self.options = [discord.SelectOption(label=player.name, value=player.userid) for player in self.game.explainers]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        if interaction.user.id == self.game.guesser.userid:
            ...
            if chosen == self.game.truth:
                ...
                self.game.lobby.cog.getUserFromDC(self.game.guesser).stats["Incorrect guesses"] += 1
                self.game.lobby.cog.getUserFromDC(chosen).stats["Fooled someone"] += 1
